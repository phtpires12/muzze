...
export const SideNav = () => {
  ...
  const handleConfirmEnd = async () => {
    ...
    const streakCount = result?.shouldShowCelebration && !result?.alreadyCounted ? result?.newStreak || 0 : 0;
  ...
  return <>
      <aside className={cn("fixed left-0 top-0 h-screen z-40 transition-all duration-300 ease-out", "bg-background/80 backdrop-blur-xl border-r border-border/50", sidebarWidth)} style={{
      paddingTop: 'env(safe-area-inset-top, 0px)'
    }} onMouseEnter={() => isInActiveSession && setIsHovering(true)} onMouseLeave={() => isInActiveSession && setIsHovering(false)}>
        <div className="flex flex-col h-full">
          ...
          <div className={cn("flex items-center h-16 border-b border-border/50 px-3", effectiveCollapsed ? "justify-center" : "justify-between")}>
            ...
            {!isInActiveSession && <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" onClick={() => setSidebarCollapsed(!isSidebarCollapsed)} className="h-8 w-8 shrink-0">
          ...
          <nav className="flex-1 p-2 space-y-1">
            {navigation.map(item => <Tooltip key={item.name}>
                ...
                {effectiveCollapsed && <TooltipContent side="right">
                    {item.name}
                  </TooltipContent>}
              </Tooltip>)}
          </nav>
          ...
          <div className="p-3 border-t border-border/50">
            <Tooltip>
              <TooltipTrigger asChild>
                <Button onClick={handleNewSession} className={cn("w-full bg-gradient-to-r from-primary to-accent hover:opacity-90 transition-all duration-300", effectiveCollapsed ? "h-10 w-10 rounded-full p-0" : "h-11 rounded-xl")}>
                  <img src={muzzeLeafWhite} alt="Nova sessão" className={cn("object-contain", effectiveCollapsed ? "w-6 h-6" : "w-5 h-5 mr-2")} />
                  {!effectiveCollapsed && <span className="font-medium">Nova Sessão</span>}
                </Button>
              </TooltipTrigger>
              {effectiveCollapsed && <TooltipContent side="right">
                  Nova Sessão
